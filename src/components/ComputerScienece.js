...
class ComputerScienece extends React.Component{
    constructor(props){
        super(props);
    this.state={
        Student:[
    ...
    render(){
        ...
        <div style={{float:"left"}}>
            <div>
                <StudentDetail bg="grey" vname="Computer Scienece"/>
            </div>
        {
          this.state.Student.map((items,index) => {
            return (
                ...
                <div className="card" style={{ width: "300px", marginLeft: "110px",marginBottom:"20px"}}>
                 ...
                 <Link to={`/ProfileOfComputerScience/${index + 1}`}><button className="btn btn-dark" style={{width:"100%"}} >Profile</button></Link>       
             </div>      
             </div>
            )
          })
...
export default ComputerScienece;